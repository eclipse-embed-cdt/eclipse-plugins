...
package org.eclipse.embedcdt.managedbuild.packs.ui.views;
...
public class DocsView extends ViewPart implements IDataManagerListener, IPropertyChangeListener {
	...
	public void createPartControl(Composite parent) {
		...
		fLatestSelectedConfig = getConfigurationForProject(EclipseUiUtils.getSelectedProject());
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (source instanceof IProject) {
			...
			if (EclipseUiUtils.getSelectedProject() == (IProject) source) {
				fLatestSelectedConfig = getConfigurationForProject(EclipseUiUtils.getSelectedProject());
	...
	private void openBook(Leaf node) throws PartInitException, IOException {
		...
		if (node.isType(Type.BOOK)) {
			...
			if (url.length() > 0) {
				...
				EclipseUiUtils.openExternalFile(path);
			}
		}
	}
