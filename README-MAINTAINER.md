...
milestone. As title, use the current version, like _v6.1.3_.
...
For all features, increase the common version (like _6.1.3_); be sure
...
use a subject like
**Embed CDT v6.1.3-202102181132 release candidate**,
...
Copy the tag and enter it in Git, like `v6.1.3-202102181132` (with `v`).
...
- commit with a message like _Eclipse Embedded CDT plug-ins v6.1.3 released_.
- push the `develop` branch
...
- tag: _v6.1.3_ (with `v`)
- title _Eclipse Embedded CDT plug-ins v6.1.3_
- copy/paste from the release page
- add `[Continue reading »](https://eclipse-embed-cdt.github.io/blog/2021/01/18/plugins-v6.1.3-released/)` with a link to the web page
- do not attach files
...
<ul>
	...
	<li><a href="https://eclipse-embed-cdt.github.io/blog/2021/01/19/plugins-v6.1.3-released/">Eclipse Embedded CDT plug-ins v6.1.3 released</a>&nbsp;(for updating the plug-ins on an existing Eclipse)</li>
</ul>
```
...
use a subject like **Eclipse Embedded CDT plug-ins v6.1.3 released**, and
...
- using the `@embedCDT` account, enter a message like
  **Eclipse Embedded CDT plug-ins v6.1.3 released** and on the next line
